...
async function extractTextFromDocx(arrayBuffer) {
  if (!window.mammoth || !window.mammoth.extractRawText) {
    throw new Error('DOCX extraction library is unavailable.');
  }
...
function isDocFile(file) {
  return file.name.toLowerCase().endsWith('.doc') || file.type === 'application/msword';
}
  ...
  reader.onload = async (e) => {
    ...
    try {
      if (isPdfFile(file)) {
        ...
        uploadText.value = text || 'Unable to extract text from this PDF. Please paste the content manually.';
        if (text) {
          showSuccessToast('PDF text extracted successfully.');
        } else {
          showInfoToast('PDF was read but produced no extractable text. Paste manually.');
        }
        return;
      }
      ...
      if (isDocxFile(file)) {
        ...
        uploadText.value = text || 'Unable to extract text from this DOCX file. Please paste the content manually.';
        if (text) {
          showSuccessToast('DOCX text extracted successfully.');
        }
        return;
      }

      if (isDocFile(file)) {
        uploadText.value = 'DOC upload is currently not automatically extracted. Please paste the text manually.';
        showInfoToast('DOC upload selected. Paste text manually.');
        return;
      }
      ...
      uploadText.value = typeof content === 'string' ? content : '';
    } catch (err) {
      console.error('File parsing error:', err);
      if (err.message && err.message.includes('PDF extraction library')) {
        uploadText.value = 'PDF upload is currently not automatically extracted. Please paste the text manually.';
        showInfoToast('PDF extraction library failed to initialize. Paste the text manually.');
      } else {
        uploadText.value = `Could not extract text from this ${extension.toUpperCase()} file. Please paste the content manually.`;
...
function loadDraft() {
  try {
    ...
    if (draft) {
      const { cv, coverLetter, cvPaid, cvId } = JSON.parse(draft);
      if (cv) latestCV = cv;
      if (coverLetter) latestCoverLetter = coverLetter;
      if (typeof cvPaid === 'boolean') hasPaid = cvPaid;
      if (cvId) latestCvId = cvId;
      if (latestCV || latestCoverLetter) {
...
async function fetchFullCv() {
  ...
  try {
    const response = await fetchWithAuth(`/cv/full/${encodeURIComponent(latestCvId)}`);
    const data = await response.json().catch(() => null);
    if (response.ok && data && data.success && data.cv) {
      latestCV = data.cv;
      saveDraft();
      updateDownloadButtons();
      setOutputText(latestCV);
    ...
    console.error('Failed to fetch full CV:', err);
  }
  return null;
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (downloadCvPdfBtn) downloadCvPdfBtn.addEventListener('click', () => {
    ...
    if (!hasPaid) {
      showFormError('Payment is required to download the full CV. Please complete payment to unlock the download.');
      return;
    }
    showFormMessage('Downloading international CV...');
  ...
  // If a saved draft exists for returning users, bypass the landing page and show results
  try {
    const rawDraft = localStorage.getItem('smartCareerDraft');
    if (rawDraft) {
      const parsed = JSON.parse(rawDraft);
      if (parsed && (parsed.cv || parsed.coverLetter)) {
        if (parsed.cv) {
          latestCV = parsed.cv;
          setOutputText(latestCV);
          showResultsSection();
          setActiveResultsTab('cv');
          if (!parsed.cvPaid) {
            showFormMessage('Your saved CV is partially shown. Pay to unlock the full version.');
          }
        } else if (parsed.coverLetter) {
          latestCoverLetter = parsed.coverLetter;
          setOutputText(latestCoverLetter);
          showResultsSection();
          setActiveResultsTab('coverLetter');
        }
      }
    }
  } catch (e) {
    console.error('Draft auto-load/bypass failed:', e);
  }
});
